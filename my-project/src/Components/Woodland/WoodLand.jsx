...
function WoodLand() {
              ...
              <div className="d-flex gap-4">
                ...
                <Col md={6}>
                  ...
                  <div style={{ display: "flex", justifyContent: "center", gap: "1rem", margin: "1rem 0" }}>
                    ...
                    <span style={{ border: "1px solid black", borderRadius: "2cm", padding: "5px 15px", color: "red" }}>
                      {quantity}
                    </span>
                    <Button
                      style={{ border: "1px solid black", backgroundColor: "white", color: "black", borderRadius: "1cm" }}
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    ...
                    <Button onClick={() => setSelectedProduct(null)} variant="secondary" className="ms-2">
                      Cancel
                    </Button>
                  </div>
                </Col>
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Woodland Shoe's Collection Online <span style={{ fontSize: "13px", color: "gray" }}>* {productwood.length} items</span>
      </h4>

      <Row className="mt-5">
        {productwood.map((wood) => {
          const isFav = favorites.some((fav) => fav.id === wood.id);

          return (
            <Col key={wood.id} lg={3} md={6} className="mb-4">
              <Card className="cus-card">
                <Card.Img src={wood.img} alt={wood.brand} />
                <Card.Title>
                  <div className="d-flex gap-2 mt-1">
                    <Button variant="info" style={{ height: "20px", paddingTop: "0" }}>
                      <h6 style={{ paddingTop: "0" }}>best seller</h6>
                    </Button>
                    <Button variant="info" style={{ height: "20px", paddingTop: "0" }}>
                      <h6 style={{ paddingTop: "0" }}>latest style</h6>
                    </Button>
                    <Button
                      variant={isFav ? "danger" : "outline-danger"}
                      style={{ height: "20px", paddingTop: "0" }}
                      onClick={() => toggleFavorite(wood)}
                    >
                      <h6 style={{ paddingTop: "0" }}>{isFav ? "♥" : "♡"}</h6>
                    </Button>
                  </div>

                  <h4 className="p-0 m-0">{wood.brand}</h4>
                  <p className="fs-6 m-0">{wood.desc}</p>
                  <h6 className="fs-5">
                    ${wood.price}{" "}
                    <span className="fs-6" style={{ color: "gray", textDecoration: "line-through" }}>
                      ${wood.oldPrice}
                    </span>{" "}
                    <span className="fs-6" style={{ color: "green" }}>
                      {wood.discount}
                    </span>
                  </h6>
                </Card.Title>
                ...
                <div className="d-flex gap-2 justify-content-center">
                  <Button variant="primary" onClick={() => handleAddToCart(wood)}>
                    Add to Cart
                  </Button>
                  <Button variant="success" onClick={() => setSelectedProduct(wood)}>
                    Shop Now
                  </Button>
                </div>
              </Card>
