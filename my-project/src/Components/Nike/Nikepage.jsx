...
function Nikepage() {
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Nike Shoe's Collection Online <span style={{ fontSize: "13px", color: "gray" }}>* 286,130 items</span>
      </h4>

      <Row className="mt-5">
        {productNike.map((nike) => {
          const isFav = favorites.some(fav => fav.id === nike.id);
          return (
            <Col key={nike.id} lg={3} md={6} className="mb-4">
              <Card className="cus-card">
                <Card.Img src={nike.img} alt={nike.brand} />
                <Card.Title>
                  <div className="d-flex gap-2 mt-1">
                    ...
                    <Button
                      ...
                      onClick={() => toggleFavorite(nike)}
                    >
                      <h6 style={{ paddingTop: "0" }}>{isFav ? "♥" : "♡"}</h6>
                    </Button>
                  </div>
                  <h4>{nike.brand}</h4>
                  <p className="fs-6 m-0">{nike.desc}</p>
                  <h6>
                    ${nike.price} <span style={{ color: "gray", textDecoration: "line-through" }}>${nike.oldPrice}</span>{" "}
                    <span style={{ color: "green" }}>{nike.discound}</span>
                  </h6>
                </Card.Title>
                <div className="d-flex gap-2 justify-content-center">
                  <Button variant="primary" onClick={() => handleAddToCart(nike)}>Add to Cart</Button>
                  <Button variant="success" onClick={() => setSelectedProduct(nike)}>Shop Now</Button>
                </div>
              </Card>
            </Col>
          );
