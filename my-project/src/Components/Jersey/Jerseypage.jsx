...
import { productJersey } from "./productJercey";
...
function Jerseypage() {
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Jersey Collection Online <span style={{ fontSize: "13px", color: "gray" }}>
          * 286,130 items
        </span>
      </h4>

      <Row className="mt-5">
        {productJersey.map((jersey) => {
          const isFav = favorites.some(fav => fav.id === jersey.id);

          return (
            <Col key={jersey.id} lg={3} md={6}>
              <Card className="cus-card mt-3">
                <Card.Img src={jersey.img} alt={jersey.brand} />
                <Card.Title>
                  <div className="d-flex gap-2 mt-1">
                    {jersey.tags.map((tag, i) => (
                      <Button key={i} variant="info" style={{ height: "20px", paddingTop: "0" }}>
                        <h6 style={{ paddingTop: "0" }}>{tag}</h6>
                      </Button>
                    ))}
                    <Button
                      variant={isFav ? "danger" : "outline-danger"}
                      style={{ height: "20px", paddingTop: "0" }}
                      onClick={() => toggleFavorite(jersey)}
                    >
                      <h6 style={{ paddingTop: "0" }}>{isFav ? "♥" : "♡"}</h6>
                    </Button>
                  </div>
                  <h4 className="p-0 m-0">{jersey.brand}</h4>
                  <p className="fs-6 m-0">{jersey.desc}</p>
                  <h6 className="fs-5">
                    ${jersey.price}{" "}
                ...
                <div className="gap-2 d-flex justify-content-center">
                  <Button variant="primary" onClick={() => handleAddToCart(jersey)}>Add to Cart</Button>
                  <Button variant="success" onClick={() => setSelectedProduct(jersey)}>Shop Now</Button>
                </div>
              </Card>
            </Col>
          );
