...
import { productRunning } from "./productRunning";
...
function Runningpage() {
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Running Shoe's Collection Online <span style={{ fontSize: "13px", color: "gray" }}>* 286,130 items</span>
      </h4>

      <Row className="mt-5">
        {productRunning.map((run) => {
          const isFav = favorites.some(fav => fav.id === run.id);
          return (
            <Col key={run.id} lg={3} md={6} className="mb-4">
              <Card className="cus-card">
                <Card.Img src={run.img} alt={run.brand} />
                <Card.Title>
                  ...
                  <h4>{run.brand}</h4>
                  <p className="fs-6 m-0">{run.desc}</p>
                  <h6>
                    ₹{run.price} <span style={{ color: "gray", textDecoration: "line-through" }}>₹{run.oldPrice}</span>{" "}
                    <span style={{ color: "green" }}>{run.discount}</span>
                  </h6>
                </Card.Title>
                <div className="d-flex gap-2 justify-content-center">
                  <Button variant="primary" onClick={() => handleAddToCart(run)}>Add to Cart</Button>
                  <Button variant="success" onClick={() => setSelectedProduct(run)}>Shop Now</Button>
                </div>
              </Card>
            </Col>
          );
