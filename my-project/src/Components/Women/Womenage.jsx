...
function Womenpage() {
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Women's Collection Online <span style={{ fontSize: "13px", color: "gray" }}>* {productWomen.length} items</span>
      </h4>

      <Row className="mt-5">
        {productWomen.map((women) => {
          const isFav = favorites.some(fav => fav.id === women.id);
          return (
            <Col key={women.id} lg={3} md={6} className="mb-4">
              <Card className="cus-card">
                <Card.Img src={women.img} alt={women.brand} />
                <Card.Title>
                  <div className="d-flex gap-2 mt-1">
                    {women.tags.map((tag, idx) => (
                      <Button key={idx} variant="info" style={{ height: "20px", paddingTop: "0" }}>
                    ...
                    <Button
                      ...
                      onClick={() => toggleFavorite(women)}
                    >
                      <h6 style={{ paddingTop: "0" }}>{isFav ? "♥" : "♡"}</h6>
                    </Button>
                  </div>
                  <h4>{women.brand}</h4>
                  <p className="fs-6 m-0">{women.desc}</p>
                  <h6>
                    ${women.price} <span style={{ color: "gray", textDecoration: "line-through" }}>${women.oldPrice}</span>{" "}
                    <span style={{ color: "green" }}>{women.discount}%</span>
                  </h6>
                </Card.Title>
                <div className="d-flex gap-2 justify-content-center">
                  <Button variant="primary" onClick={() => handleAddToCart(women)}>Add to Cart</Button>
                  <Button variant="success" onClick={() => setSelectedProduct(women)}>Shop Now</Button>
                </div>
              </Card>
            </Col>
          );
