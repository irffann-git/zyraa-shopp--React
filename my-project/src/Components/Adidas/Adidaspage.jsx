...
import { productAdidas } from "./productAdidas"; 
...
function Adidaspage() {
  ...
  const handlePlaceOrder = () => {
    alert(`Order placed for ${quantity} ${selectedProduct.brand} shoes!`);
    setQuantity(1);
  };
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Adidas Shoe's Collection Online{" "}
        <span style={{ fontSize: "13px", color: "gray" }}>* 286,130 items</span>
      </h4>

      <Row className="mt-5">
        {productAdidas.map((adidas) => {
          const isFav = favorites.some((fav) => fav.id === adidas.id);

          return (
            <Col key={adidas.id} lg={3} md={6}>
              <Card className="cus-card mt-3">
                <Card.Img src={adidas.img} alt={adidas.brand} />
                <Card.Title>
                  <div className="d-flex gap-2 mt-1">
                    ...
                    <Button
                      ...
                      onClick={() => toggleFavorite(adidas)}
                    >
                      <h6 style={{ paddingTop: "0" }}>{isFav ? "♥" : "♡"}</h6>
                    </Button>
                  </div>
                  <h4 className="p-0 m-0">{adidas.brand}</h4>
                  <p className="fs-6 m-0">{adidas.desc}</p>
                ...
                <div className="gap-2" style={{ display: "flex", justifyContent: "center" }}>
                  <Button variant="primary" onClick={() => handleAddToCart(adidas)}>Add to Cart</Button>
                  <Button variant="success" onClick={() => setSelectedProduct(adidas)}>Shop Now</Button>
                </div>
              </Card>
            </Col>
          );
