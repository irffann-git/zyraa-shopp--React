...
function Manpage() {
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Men's Collection Online{" "}
        <span style={{ fontSize: "13px", color: "gray" }}>* 286,130 items</span>
      </h4>

      <Row className="mt-5">
        {productMen.map((man) => {
          const isFav = favorites.some((fav) => fav.id === man.id); 
          return (
            <Col key={man.id} lg={3} md={6} className="mb-4">
              <Card className="cus-card">
                <Card.Img src={man.img} alt={man.brand} />
                <Card.Title>
                  <div className="d-flex gap-2 mt-1">
                    ...
                    <Button
                      ...
                      onClick={() => toggleFavorite(man)}
                    >
                      <h6 style={{ paddingTop: "0" }}>{isFav ? "♥" : "♡"}</h6>
                    </Button>
                  </div>
                  <h4>{man.brand}</h4>
                  <p className="fs-6 m-0">{man.desc}</p>
                  <h6>
                    ${man.price} <span style={{ color: "gray", textDecoration: "line-through" }}>${man.oldPrice}</span>{" "}
                    <span style={{ color: "green" }}>{man.discount}</span>
                  </h6>
                </Card.Title>
                <div className="d-flex gap-2 justify-content-center">
                  <Button variant="primary" onClick={() => handleAddToCart(man)}>
                    Add to Cart
                  </Button>
                  <Button variant="success" onClick={() => setSelectedProduct(man)}>
                    Shop Now
                  </Button>
                </div>
              </Card>
