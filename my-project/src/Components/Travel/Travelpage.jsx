...
function Travelpage() {
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Travel Collection Online <span style={{ fontSize: "13px", color: "gray" }}>* 286,130 items</span>
      </h4>

      <Row className="mt-5">
        {productTravel.map((travel) => {
          const isFav = favorites.some(fav => fav.id === travel.id);
          return (
            <Col key={travel.id} lg={3} md={6} className="mb-4">
              <Card className="cus-card">
                <Card.Img src={travel.img} alt={travel.brand} />
                <Card.Title>
                  ...
                  <h4>{travel.brand}</h4>
                  <p className="fs-6 m-0">{travel.desc}</p>
                  <h6>
                    ${travel.price} <span style={{ color: "gray", textDecoration: "line-through" }}>${travel.oldPrice}</span>{" "}
                    <span style={{ color: "green" }}>{travel.discount}</span>
                  </h6>
                </Card.Title>
                <div className="d-flex gap-2 justify-content-center">
                  <Button variant="primary" onClick={() => handleAddToCart(travel)}>Add to Cart</Button>
                  <Button variant="success" onClick={() => setSelectedProduct(travel)}>Shop Now</Button>
                </div>
              </Card>
            </Col>
          );
