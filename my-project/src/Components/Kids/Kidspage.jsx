...
function Kidspage() {
  ...
  return (
    <Container className="mt-4 mb-3">
      <h4>
        Buy Kid's Collection Online <span style={{ fontSize: "13px", color: "gray" }}>* 286,130 items</span>
      </h4>

      <Row className="mt-5">
        {productKids.map((kids) => {
          const isFav = favorites.some(fav => fav.id === kids.id);
          return (
            <Col key={kids.id} lg={3} md={6} className="mb-4">
              <Card className="cus-card">
                <Card.Img src={kids.img} alt={kids.brand} />
                <Card.Title>
                    ...
                    <Button variant="info" style={{ height: "20px", paddingTop: "0" }}>
                     ...
                     <Button variant="info" style={{ height: "20px", paddingTop: "0" }}>
                      ...
                      <Button  variant={isFav ? "danger" : "outline-danger"} style={{ height: "20px", paddingTop: "0" }}
                        onClick={() => toggleFavorite(kids)}>
                         <h6 style={{ paddingTop: "0" }}>{isFav ? "♥" : "♡"}</h6></Button>
                   </div>
                  <h4>{kids.brand}</h4>
                 <p className="fs-6 m-0">{kids.desc}</p>
                  <h6>
                    ${kids.price} <span style={{ color: "gray", textDecoration: "line-through" }}>${kids.oldPrice}</span>{" "}
                    <span style={{ color: "green" }}>{kids.discount}</span>
                  </h6>
                </Card.Title>
                <div className="d-flex gap-2 justify-content-center">
                  <Button variant="primary" onClick={() => handleAddToCart(kids)}>Add to Cart</Button>
                  <Button variant="success" onClick={() => setSelectedProduct(kids)}>Shop Now</Button>
                </div>
              </Card>
            </Col>
          );
